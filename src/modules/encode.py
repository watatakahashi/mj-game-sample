...
# 捨て牌と副露のエンコード
def encode_discard(myPlayerDiscard, lowerPlayerDiscard, oppositePlayerDiscard, upperPlayerDiscard):
...
def encode_player(category_num):
    return np.array([np.ones(37) if i == category_num else np.zeros(37) for i in range(4)])

# リーチのエンコード
def encode_reach(category_list):
    return np.array([np.ones(37) if i == 1 else np.zeros(37) for i in category_list])

# 場風のエンコード
def encode_bakaze(category_num):
    return np.array([np.ones(37) if i == category_num else np.zeros(37) for i in range(3)])
...
def encode_point(my_point, target_point):
    ...
    bins = [-12000, -10000, -8000, -4000, -1000, 0, 1000, 4000, 8000, 10000, 12000]
    which_bin = np.digitize(diff, bins=bins)
    point_array = [np.ones(37) if i == which_bin else np.zeros(37) for i in range(len(bins) + 1)]
