...
def convert_to_forth_dimension(pai_type, tehai):
    ...
    elif count == 1:return[1,0,0,0]
    elif count == 2:return[1,1,0,0]
    elif count == 3:return[1,1,1,0]
    elif count == 4:return[1,1,1,1]
...
def convert(tehai):
    mapped = list(map(lambda pai_type: convert_to_forth_dimension(pai_type, tehai), PAI_LIST))
    return mapped
...
def convert_safety(safety_str):
    # 01変換
    one_hot_func = lambda pai_type, pai_str: 1 if pai_type in pai_str else 0
    # 37種類を変換
    mapped = list(map(lambda pai_type: one_hot_func(pai_type, safety_str), PAI_LIST))
