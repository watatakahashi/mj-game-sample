...
class PaihuDataSequence(Sequence):
    ...
    def __getitem__(self, i):
        sample_df = self.df.iloc[i * self.batch_size: (i + 1) * self.batch_size, :]

        # 手牌
        tehai_feature = np.array(list(map(encode.encode_tehai,sample_df.privateTehaiString.values)))
        ...
        # ドラ表示牌    
        dora_feature = np.array(list(map(encode.encode_dora,sample_df.doraOpen.values)))

        # プレイヤー
        player_feature = np.array(list(map(encode.encode_player, sample_df.player.values)))
        ...
        # 場風
        bakaze_feature = np.array(list(map(encode.encode_bakaze,sample_df.bakaze.values)))
        
        # 局数
        kyoku_feature = np.array(list(map(encode.encode_kyoku,sample_df.kyokuNum.values)))
        ...
        sample_Y = sample_df.selectedPai.values
        Y = np.array(list(map(convert_selected_pai_to_num, sample_Y)))
        return X, Y
    ...
    def on_epoch_end(self):
        ...
        if self.vmstat: print(subprocess.getoutput('vmstat -S m'))